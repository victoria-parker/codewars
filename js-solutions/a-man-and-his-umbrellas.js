...
function minUmbrellas(weather) {
    ...
    for(let i=0;i<weather.length;i++){
        //First Morning
        if(i==0 && (weather[i]=='rainy' || weather[i]=='thunderstorms')){
            stockAtWork.push(1)   
        }
        //Afternoon (odd index)
        if(i % 2 != 0 && (weather[i]=='rainy' || weather[i]=='thunderstorms')){
            if(stockAtWork.length > 0){
                stockAtWork.pop()
                stockAtHome.push(1)
            }else{
                stockAtHome.push(1)
        ...
        //Mornings (even index)
        if(i!= 0 && i % 2 == 0 && (weather[i]=='rainy' || weather[i]=='thunderstorms')){
            if(stockAtHome.length > 0){
                stockAtHome.pop()
                stockAtWork.push(1)
            }else{
                stockAtWork.push(1)
    ...
    return [...stockAtHome,...stockAtWork].length
}

console.log(minUmbrellas(['rainy','sunny','thunderstorms','rainy','clear','rainy','thunderstorms','thunderstorms','rainy','rainy' ]))// 2
